...
function EndpointCard({ endpoint, apiId }: { endpoint: ApiEndpoint; apiId: string }) {
    ...
    <div className="bg-white border border-bolivar-border rounded-xl overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-bolivar-border p-4 space-y-4">
          ...
          <Link to={`/sandbox?apiId=${apiId}&endpoint=${encodeURIComponent(endpoint.path)}&method=${endpoint.method}`} className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
            <FlaskConical className="w-4 h-4" /> Probar en Sandbox
          </Link>
        </div>
      )}
    </div>
...
export default function ApiDetailPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <Link to="/catalog" className="inline-flex items-center gap-1 text-sm text-primary hover:underline mb-6">
      ...
      <div className="space-y-3">
        {api.endpoints.map((ep, i) => (
          <EndpointCard key={i} endpoint={ep} apiId={api.id} />
        ))}
      </div>
    </div>
  );
